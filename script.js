...
function renderScenarioOptions(scenario) {
  return scenario.options
    .map((option, optionIndex) => {
      const roleChips = option.dominantRoles
        .map((roleName) => `<span class="option-role role-chip">${roleName}</span>`)
        .join("");

      return `
        <button class="option-button" type="button" data-action="choose-option" data-index="${optionIndex}" ${appState.awaitingContinue ? "disabled" : ""}>
          <div>
          ...
          <div class="option-stats">
            <div class="option-meta">${roleChips}</div>
          </div>
        </button>
      `;
    })
...
function renderScenarioSidebar() {
  const roleSummary = getRoleSummary();
  const recentDecision = appState.history[appState.history.length - 1];
  const recentRoles = recentDecision ? recentDecision.dominantRoles.join(" / ") : "No choices yet";
  ...
  return `
    <div class="phase-side">
      ...
      <div class="signal-card">
        ...
        <p class="note-copy">Most recently amplified roles: ${recentRoles}</p>
        <p class="note-copy">Current balance signal: ${roleSummary.balanceLabel}</p>
      </div>

      <div class="signal-card">
        <strong>Pressure Signal</strong>
        <p class="note-copy">High momentum can still hide a weakening system. Watch whether trust, control, and viability are holding underneath the story.</p>
      </div>
    </div>
  `;
}
    ...
    <section class="screen">
      <div class="phase-grid">
        <div class="phase-main">
          ...
          <div class="panel-card">
            <div class="section-kicker">Role Prompts</div>
            <p class="section-copy">Use each team-role lens before deciding. In Collapse, some of the most valuable voices are also the easiest to ignore.</p>
            <div class="prompt-grid">${renderRolePromptGrid(scenario.phase)}</div>
          </div>
